...
def load_pickle(pth):
    ...
    - Open the file at pth path in binary read mode
    - Load the pickled data from the file using pickle.load()
...
def save_model(ckpt, model, nickname, save_path):
    ...
    Args:
        ...
        nickname: Name to use for model in checkpoint
        save_path: Path to save checkpoint file to
    Returns: 
        None: Function saves checkpoint and returns nothing
    - Extract model state dict 
    - Add state dict to checkpoint under nickname key
...
def main(MODEL_HOME, MODEL_PATH, SAVE_PATH):
    ...
    Processing Logic:
        ...
        - Load label embeddings from GloVe/fastText
        - Initialize linear mapping model
        - Optimize model via SGD to minimize MSE loss between predicted and actual image embeddings
