...
def get_base_labels(pth):
    ...
    - Initialize empty list of same length as dictionary
...
def get_classifier_weights(pth, device):
    ...
    - Load checkpoint file to a dict using torch.load()
    - Return entire checkpoint dict and classifier weights nested in the dict"""
...
def save_model(ckpt, model, nickname, save_path):
    ...
    - Saves it to the checkpoint dictionary with the given nickname 
    - Saves the entire checkpoint dictionary to the given save path"""
...
def main(MODEL_HOME, MODEL_PATH, SAVE_PATH):
    ...
    Args:
        ...
        MODEL_PATH: Path to pretrained image classifier model
        SAVE_PATH: Path to save trained linear mapping model
    Returns: 
        None
    
    Processes Logic:
        - Loads pretrained image classifier weights and label embeddings
        - Initializes a linear mapping model 
        - Trains the linear mapping model to minimize MSE loss between predicted and actual image embeddings
        - Saves the trained linear mapping model
    """
