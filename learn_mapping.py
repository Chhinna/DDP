...
def get_classifier_weights(pth, device):
    ...
    Loads PyTorch checkpoint, returns full checkpoint and classifier weights:
        - Load checkpoint from pth using device for mapping
        - Return full checkpoint and classifier weights
    """
...
def save_model(ckpt, model, nickname, save_path):
    ...
    - Add model state dict to checkpoint dictionary using nickname as key
    - Save checkpoint dictionary to file at save_path
    """
...
def main(MODEL_HOME, MODEL_PATH, SAVE_PATH):
    ...
    Processing Logic:
        ...
        - Define linear mapping model to map from label embedding size to image embedding size  
        - Optimize model via SGD with MSE loss to minimize distance between mapped label embeddings and image embeddings
