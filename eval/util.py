...
def drop_a_dim(data): #TODO why do we need this in the first place?
    ...
    Returns: 
        Tuple: Dropped dimension tensors containing support and query data.
    Processing Logic:
        - View support and query tensors to remove batch dimension
        - View support and query labels to remove batch dimension
        - Return tuple of processed tensors and labels
    """
...
def get_batch_cycle(meta_trainloader_it, meta_trainloader):
    ...
    - If iterator is exhausted, reset it with dataloader iterator
    - Return next batch"""
    try:
