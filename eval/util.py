...
def normalize(x):
    ...
    - Take the sum of squares of elements along dimension 1 and keep the dimension
...
def image_formatter(im):
    ...
    Processing Logic:
        ...
        - Return HTML image tag with data URI
    """
...
def freeze_backbone_weights(backbone, opt, epoch, exclude=['classifier.transform']):
    ...
    Freeze backbone weights at specified epoch:
...
def get_optim(net, opt):
    ...
    Initializes either an Adam or SGD optimizer based on opt.adam flag:
        ...
        - Else initializes SGD optimizer using opt.learning_rate, opt.momentum, opt.weight_decay"""
    if opt.adam:
...
def get_vocab(loaders):
    ...
    - Filter out empty strings and add remaining names to a vocabulary list
    - Append each vocabulary to a list of vocabularies
