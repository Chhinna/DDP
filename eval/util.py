...
class AverageMeter(object):
    ...
    def __init__(self):
        ...
        - Initialize any other properties of the object"""
        self.reset()

    def reset(self):
        """Resets the statistics tracker to initial values.
        Args:
            self: The statistics tracker object. 
        Returns: 
            None: Does not return anything.
        - Sets the value being tracked (val) to 0
        - Sets the running average (avg) to 0 
...
def mean_confidence_interval(data, confidence=0.95):
    """
    Calculate a mean confidence interval for a data set
    Args:
        data: Data to calculate confidence interval for
        confidence: Confidence level for interval in decimal format (default 0.95)
    Returns: 
    ...
    Processing Logic:
        ...
        - Return mean and half-width to define confidence interval range as m ± h"""
    a = 1.0 * np.array(data)
    n = len(a)
...
def image_formatter(im):
    """
    Formats image for embedding in HTML.
    Args:
        im: Image array to format
    Returns: 
        HTML string: Formatted image embedded in HTML
    1. Normalize image pixels to range 0-255
    2. Convert to uint8 and transpose dimensions 
    3. Save image to BytesIO stream as PNG
    4. Encode PNG as base64 string for embedding in HTML
    5. Return HTML string with embedded image
    """
...
def freeze_backbone_weights(backbone, opt, epoch, exclude=['classifier.transform']):
    ...
    Freeze weights of backbone model except for specified layers:
        - Check if current epoch matches option to freeze backbone
        - Set requires_grad to False for all backbone parameters
        - Check for excluded layers and set requires_grad to True for them
        - Print out layers that were not frozen
    """
...
def get_optim(net, opt):
    ...
    - Checks if Adam optimizer requested
    - If yes, initializes Adam optimizer on network parameters
    - If no, initializes SGD optimizer on network parameters
...
def get_vocabs(base_loader=None, novel_loader=None, query_ys=None):
    ...
    Args:
        ...
        query_ys: Query labels to get novel ids from
    Returns: 
        vocab_base: Base vocabulary
        vocab_all: Concatenated base and novel vocabularies
        vocab_novel: Novel vocabulary
        orig2id: Mapping from original novel ids to indices in vocab_all
    Processing Logic:
        - Extract base vocabulary from base loader
        - Extract novel ids from query labels
        - Extract novel vocabulary from novel loader using novel ids
        - Concatenate base and novel vocabularies
        - Return vocabularies and id mapping
    """
    vocab_all = []
    vocab_base = None
...
def drop_a_dim(data): #TODO why do we need this in the first place?
    ...
    Args:
        data: Tuple of tensors containing the support and query data.
    Returns: 
        Tuple: Tuple containing the support and query data with one dimension dropped.
    Processing Logic:
        - View support_xs to remove batch dimension
        - View query_xs to remove batch dimension 
        - View support_ys and query_ys to remove batch dimension
    """
...
def get_batch_cycle(meta_trainloader_it, meta_trainloader):
    ...
    - If iterator is exhausted, reset it to iterator over dataloader
    - Return next batch"""
    try:
