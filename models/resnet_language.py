...
class LinearMap(nn.Module):
    def __init__(self, indim, outdim):
        ...
        - Stores the linear layer as an attribute for future use in forward pass"""
...
#         self.novel_embeds = torch.cat((self.novel_embeds, new_novel_embeds), 0)
    ...
    def create_pulling_mapping(self, state_dict, base_weight_size=640):
        ...
        - Loads pretrained linear mapping model from state dict
        - Sets input and output dimensions of mapping model  
        - Moves mapping model to GPU
        - Attaches mapping model to self
        """
    ...
    def forward(self, base_weight, mask=False):
        ...
        Returns: 
            scores: Fine-tuned embeddings pulled from base weights.
        Processing Logic:
            - Compute similarity between novel and base embeddings.
            - Apply softmax with temperature. 
            - Mask diagonal in similarity matrix if specified.
            - Pull base weights using similarity matrix and return fine-tuned embeddings.
        """
...
class ResNet(nn.Module):
    ...
    def reglossnovel(self, lmbd, novel_weight, novel_bias=None):
        ...
        Processing Logic:
            - Calculates L2 norm between novel class weights in classifier and novel_weight
            - If novel_bias is provided, calculates L2 norm between novel class biases in classifier and novel_bias 
            - Returns sum of both regularization losses
        """
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.0, drop_block=False,
        ...
        Args:
            inplanes: {Input feature maps}: Number of input feature maps
            planes: {Output feature maps}: Number of output feature maps 
            stride: {Downsampling stride}: Stride for downsampling
            downsample: {Downsampling layer}: Downsampling layer if needed
            drop_rate: {Dropout rate}: Dropout rate
            drop_block: {Use DropBlock}: Use DropBlock if True
            block_size: {DropBlock size}: Size of DropBlock if used
            use_se: {Use Squeeze-and-Excitation}: Use SE if True
        Returns: 
            self: {Initialized object}
        Processing Logic:
            - Apply 3x3 convolution, batch normalization and ReLU on input
            - Apply 3x3 convolution, batch normalization and ReLU
            - Apply 3x3 convolution, batch normalization
            - Apply max pooling if downsampling is needed
            - Initialize downsample layer if provided
            - Initialize attributes like stride, dropout, etc.
            - Initialize DropBlock layer if used
            - Initialize Squeeze-and-Excitation layer if used
        """
    ...
    def forward(self, x):
        ...
        - Passes x through 3 convolution + batch norm + ReLU layers
        - Performs downsampling if necessary 
        - Adds residual to output
        - Applies final ReLU and maxpool
        - Optionally applies DropBlock or dropout
        """
...
class DropBlock(nn.Module):
    ...
    def _compute_block_mask(self, mask):
        ...
        Processing Logic:
            ...
            - Create offsets tensor for block indices
            ...
            - Return block masked version by inverting padded mask
        """
