...
class LinearMap(nn.Module):
    def __init__(self, indim, outdim):
        ...
        - Initializes the parent class with super()
        - Saves the linear layer as an attribute self.map"""
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, n_blocks, keep_prob=1.0, avg_pool=False, drop_rate=0.0,
        ...
        Args:
            ...
            avg_pool: {Use average pooling or not}
            drop_rate: {Dropout rate}
            dropblock_size: {DropBlock size}
            num_classes: {Number of classes to classify}
            use_se: {Use Squeeze-and-Excitation or not}
            vocab: {Vocabulary (for text classification)} 
            opt: {Options}
        Returns:
            self: {Initialized ResNet model}
        Processing Logic:
            1. Defines the initial parameters like inplanes, use_se etc.
            2. Calls _make_layer to create each layer with the given parameters  
            3. Adds average pooling or dropout 
            4. Initializes weights
            5. Adds classification head if num_classes is specified
        """
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.0, drop_block=False,
        ...
        Processing Logic:
            ...
            - Initialize other parameters like drop rate, drop block etc
        """
