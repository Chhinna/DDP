...
class LinearMap(nn.Module):
    def __init__(self, indim, outdim):
        ...
        - Initializes a linear layer using nn.Linear to map from input to output dimensions
        - Calls the parent class' __init__ to complete initialization
    ...
    def forward(self, x):
        ...
        - Apply forward transformation of layer by calling self.map on input x
        - self.map contains layer specific forward transformation 
...
class LangPuller(nn.Module):
    def __init__(self,opt, vocab_base, vocab_novel):
        ...
        Processing Logic:
            ...
            - Initializes softmax for label attractor computation
...
#         self.novel_embeds = torch.cat((self.novel_embeds, new_novel_embeds), 0)
    ...
    def create_pulling_mapping(self, state_dict, base_weight_size=640):
        ...
        Returns: 
            self: Instance with mapping model loaded
        ...
        - Attaches loaded mapping model to instance
        """
    ...
    def forward(self, base_weight, mask=False):
        ...
        Returns: 
            scores: Fine-tuned weights for novel classes
        Processing Logic:
            - Compute similarity between novel and base class embeddings
            - Apply softmax with temperature
            - Mask the diagonal if specified
            - Multiply similarities and base weights
            - Alternatively, do a linear mapping with the mapping model
        """
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, drop_rate=0.0, drop_block=False,
        ...
        Args:
            ...
            stride: {Stride size for the first convolutional layer. Default=1}
            downsample: {Downsample factor for the shortcut. Default=None}
            drop_rate: {Dropout rate. Default=0.0}
            drop_block: {Use DropBlock instead of Dropout. Default=False}
            block_size: {DropBlock size. Only used if drop_block is True. Default=1}
            use_se: {Use Squeeze-and-Excitation. Default=False}
        Returns: 
            self: {Initialized BasicBlock module}
        Processing Logic:
            - Apply 1x1 convolution on input
            ...
            - Apply Squeeze-and-Excitation if use_se is True
            - Add shortcut and pass through normalization
        """
