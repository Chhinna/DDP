...
class LinearMap(nn.Module):
    ...
    def forward(self, x):
        ...
        Args: 
            x: Input tensor 
        Returns:
            output: Transformed input tensor by applying forward pass of layer
        ...
        - It transforms x based on layer parameters and returns output"""
        return self.map(x)
...
class LangPuller(nn.Module):
    def __init__(self,opt, vocab_base, vocab_novel):
        ...
        Processing Logic:
            ...
            - Retrieves base and novel embeddings from file paths
    ...
    def update_novel_embeds(self, vocab_novel):
        ...
        Processing Logic:
            ...
            - Load novel embeddings into GPU memory
            - Assign novel embeddings to model's novel embedding attribute
            - If using GloVe, only take first 300 dimensions of novel embeddings
        """
        # Retrieve novel embeds
        opt = self.opt
...
#         self.novel_embeds = torch.cat((self.novel_embeds, new_novel_embeds), 0)
    ...
    def create_pulling_mapping(self, state_dict, base_weight_size=640):
        ...
        - Sets input and output dimensions of LinearMap based on novel embedding and base weight sizes  
    ...
    def loss1(pull, inspired, weights):
        ...
        Args:
            pull: Scalar value to multiply loss
            inspired: Input tensor for comparison 
            weights: Weights tensor for comparison
        Returns: 
            Loss value: Scalar loss value between pull and distance
        - Calculates L2 norm of difference between inspired and weights tensors  
        - Multiplies L2 norm by pull scalar
        - Returns pull * L2_norm as loss"""
...
class ResNet(nn.Module):
    ...
    def _make_layer(self, block, n_block, planes, stride=1, drop_rate=0.0, drop_block=False, block_size=1):
        ...
        Returns:
            Sequential: {Sequential layer containing stacked blocks}
        Processing Logic:
            - Creates downsample layer if needed
            - Adds first block with specified stride
            - Adds remaining blocks with stride=1
            - Returns Sequential layer containing stacked blocks
        """
        downsample = None
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        - Passes x through 1x1, 3x3, 1x1 convolutions and batchnorm
        - Adds residual and output and applies ReLU
        - Optionally downsample spatial size and applies maxpool
        - Optionally applies DropBlock/Dropout
        """
