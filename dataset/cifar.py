...
class CIFAR100(Dataset):
    ...
    def __init__(self, args, partition='train', pretrain=True, is_sample=False, k=4096,
        ...
        preprocess_contrastive_sampling(self)
    ...
    def preprocess_contrastive_sampling(self):
        if self.is_sample:
...
class MetaCIFAR100(CIFAR100):
    ...
    def assign_images_to_classes(self):
        self.data = {}
    ...
    def __getitem__(self, item):
        ...
        """
        support_xs, support_ys, query_xs, query_ys = sample_classes_and_split_data(self, item)
        support_xs, support_ys, query_xs, query_ys = np.array(support_xs), np.array(support_ys), np.array(
            query_xs), np.array(query_ys)
        support_xs, query_xs, support_ys, query_ys = reshape_and_split_data(query_xs, query_ys, support_xs, self, support_ys)
    
        support_xs, query_xs = apply_transformations(self, support_xs, query_xs)
    
        return support_xs, support_ys, query_xs, query_ys
    
    def sample_classes_and_split_data(self, item):
        if self.fix_seed:
            np.random.seed(item)
    ...
    def reshape_and_split_data(query_xs, query_ys, support_xs, self, support_ys):
